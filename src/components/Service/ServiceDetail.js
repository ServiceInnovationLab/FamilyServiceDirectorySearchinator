  ...
  loadServiceDetails(serviceId) {
    if (!this.state.recordsLoaded) {
      this.setState({
        ...
        localcategory: this.props.searchVars.category,
      });
      const FIELDS =
      ...
      let urldetails = encodeURI(
        `${process.env.REACT_APP_API_PATH}datastore_search?resource_id=${
          process.env.REACT_APP_API_RESOURCE_ID
        }&fields=${FIELDS}${this.filters(serviceId)}`,
      );
  ...
  displayServiceDetails(category) {
    ...
    if (this.state.records.length > 0) {
      ...
      this.state.records.forEach(function(item) {
        ...
        if (item.LEVEL_1_CATEGORY === currentCategory)
