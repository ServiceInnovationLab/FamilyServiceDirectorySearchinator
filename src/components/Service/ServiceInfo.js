// Modules
import React from 'react';
import { connect } from 'react-redux';
import { Route, Link } from 'react-router-dom';

// Local components
import MapResults from '../Map/MapResults';
import Sharebar from '../Social/Sharebar';
import ServiceDetail from './ServiceDetail';
import ServiceContactDetail from './ServiceContactDetail';

// Data
import * as actionCreators from '../../actions/index';

export class ServiceInfo extends React.Component {
  componentDidMount = () => {
    const { loadService, match } = this.props;
    const { category, name } = match.params;

    loadService({ category }, name);
    this.goToTop();
  };

  goToTop = () => {
    window.scrollTo(0, 0);
  };

  backLink = () => {
    const { searchVars } = this.props;
    let url = '/';

    if (searchVars.category) url += `category/${searchVars.category}`;
    return url;
  };

  render() {
    const {
      match: { params: { name } },
      result,
      searchVars,
      changeCategory
    } = this.props;
    const filteredResults = result.filter(item => item.FSD_ID === name);

    return (
      <div>
        <div className="container-fluid">
          <div
            className={
              'service-info' + (filteredResults.length === 0 ? ' loading' : '')
            }
          >
            <Route
              render={() => (
                <Link to={this.backLink} onClick={this.goToTop}>
                  Go back
                </Link>
              )}
            />
            <ul className="list-stripped">
              {filteredResults.map((i, key) => (
                <div key={key}>
                  <div className="search-result-hero">
                    <h2>{i.PROVIDER_NAME}</h2>
                    <ServiceContactDetail
                      phone={i.PUBLISHED_PHONE_1}
                      email={i.PUBLISHED_CONTACT_EMAIL_1}
                      hours={i.PROVIDER_CONTACT_AVAILABILITY}
                    />
                  </div>
                  <ServiceDetail
                    results={i}
                    changeCategory={changeCategory}
                    searchVars={searchVars}
                    serviceId={i.FSD_ID}
                    loadimmediately={true}
                    preview={false}
                  />
                  <Sharebar subject={i.PROVIDER_NAME} />
                </div>
              ))}
            </ul>
          </div>
        </div>
        <MapResults map_results={filteredResults} />
      </div>
    );
  }
}

export default connect(
  state => ({ result: state.result, searchVars: state.searchVars }),
  actionCreators
)(ServiceInfo);
